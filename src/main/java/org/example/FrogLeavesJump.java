...
public class FrogLeavesJump {

	public int solution(int X, int[] A) {
        int[] leaves = new int[X]; //Keep here minimal time when leaf reched river
        for(int i = 0; i < X; i++) leaves[i]=-1;
        int minTime = Integer.MIN_VALUE;
        for(int i=0; i<A.length; i++) {
            if (1 <= A[i] && A[i] <= X) { //We found the position
                if (leaves[A[i]-1] == -1) leaves[A[i]-1]=i; //Initiate leaf
                else
                    leaves[A[i]-1] = (leaves[A[i]-1] > i) ? i : leaves[A[i]-1]; //Choose the minimum (i here is a time)
            }
        }
        for(int i = 0; i<X; i++) {
            if (leaves[i] == -1) return -1; //Leaf is missing
            else {
                minTime = (minTime < leaves[i]) ? leaves[i] : minTime; //Find maximum time
            }    
        }
        return minTime;
	}
